...
def handle_unhandled_exception(e):
    """Global catch-all exception handler to prevent stack trace leakage."""
...
def handle_sqlalchemy_exception(e):
    """Dedicated handler for database-related exceptions."""
    try:
        db.session.rollback()
...
def handle_csrf_error(e):
    """Custom CSRF error handler returning JSON."""
...
def index():
    ...
    endpoints_info = {
        ...
        "endpoints": {
            ...
            "GET /api/books/<id>/bookmarked": "Check if a book is bookmarked",
        },
...
def login():
    ...
    try:
        ...
        if user and user.check_password(password):
            access_token = create_access_token(identity=str(user.id))
            resp, status = success_response(data={
    ...
    try:
        ...
        if not user:
            ...
            if user:
                ...
                base_username = ''.join(
                    ch for ch in username.strip().replace(' ', '_') if ch.isalnum() or ch == '_'
                )[:50]
...
def get_collection_books(collection_id):
    ...
    try:
        ...
        if not collection.is_public and str(collection.user_id) != str(current_user_id):
            return forbidden_error("Unauthorized")
        items = CollectionItem.query.filter_by(collection_id=collection_id).order_by(
...
def create_bookmark():
    ...
    try:
        ...
        is_valid, validated_data = validate_request(BookmarkRequest, payload)
        if not is_valid:
            return jsonify(validated_data), 400

        book = Book.query.filter_by(id=validated_data.book_id, is_deleted=False).first()
        if not book:
            return not_found_error("Book not found")

        existing = Bookmark.query.filter_by(
            user_id=user_id, book_id=validated_data.book_id, is_deleted=False
        ).first()
        if existing:
        ...
        logger.info(f"User {user_id} bookmarked book {validated_data.book_id}")
...
def get_bookmark(bookmark_id):
    ...
    try:
        ...
        bookmark = Bookmark.query.filter_by(
            id=bookmark_id, user_id=user_id, is_deleted=False
        ).first()
        if not bookmark:
...
def update_bookmark(bookmark_id):
    ...
    try:
        ...
        is_valid, validated_data = validate_request(UpdateBookmarkRequest, payload)
        if not is_valid:
            return jsonify(validated_data), 400
        ...
        if validated_data.page_number is not None:
            bookmark.page_number = validated_data.page_number
...
def check_bookmark(book_id):
    ...
    try:
        user_id = get_jwt_identity()
        bookmark = Bookmark.query.filter_by(
            user_id=user_id, book_id=book_id, is_deleted=False
        ).first()
        return success_response({
...
if __name__ == '__main__':
    ...
    if server_config.debug:
        ...
        logger.info("  POST /api/bookmarks - Create bookmark")
        logger.info("  GET  /api/bookmarks - List bookmarks")
