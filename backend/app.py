...
try:
    ...
    logger.warning(
        "Mood analysis package not available - some endpoints will be disabled"
    )
...
# flake8: noqa: E501
current_user_id = (
    None  # Placeholder for user context, to be set in request hooks if needed
)
validated_data = (
    None  # Placeholder for request data validation function, to be defined later
)
server_config = None  # Placeholder for server configuration, to be loaded from environment variables
...
def handle_unhandled_exception(e):
    ...
    try:
        ...
        if isinstance(e, SQLAlchemyError):
            db.session.rollback()
            logger.error(
                "Rolled back database session due to SQLAlchemyError in global handler."
            )
    except ImportError:
        pass
...
def handle_sqlalchemy_exception(e):
    ...
    try:
        ...
        logger.critical(
            f"Failed to rollback DB session during SQLAlchemyError handling: {rollback_err}"
        )
...
def handle_csrf_error(e):
    ...
    """
    logger.warning(
        f"CSRF Validation Failed: {e.description} | Remote IP: {request.remote_addr}"
    )
    return (
        jsonify(
            {
...
def _validate_jwt_secret_startup():
    ...
    if not is_valid:
        ...
        if app_config.is_development():
            ...
            logger.info(
                f"Rate limiting: {'Enabled' if app_config.rate_limit.enabled else 'Disabled'}"
            )
            logger.info("=" * 70)
...
def health_check():
    ...
    if not app_config.is_production():
        ...
        is_using_dummy_keys = (
            app_config.ai_service.openai_api_key == "test-dummy-openai-key"
            or app_config.ai_service.groq_api_key == "test-dummy-groq-key"
            or app_config.email.api_key == "test-dummy-email-key"
        )

        health_status["diagnostics"] = {
            "db_url_scheme": (
                str(app_config.database.url).split("://")[0]
        ...
        if (
            app_config.get_environment_name() in ["test", "testing"]
            and not is_using_dummy_keys
        ):
...
def index():
    ...
    if MOOD_ANALYSIS_AVAILABLE:
        ...
        ] = "Analyze book mood from GoodReads"
        endpoints_info["endpoints"][
            "POST /api/v1/mood-tags"
        ] = "Get mood tags for a book"
    else:
        endpoints_info[
            "note"
        ] += " | Mood analysis endpoints disabled (missing dependencies)"
...
def handle_analyze_mood(validated_data):
    ...
    if not MOOD_ANALYSIS_AVAILABLE:
        return service_unavailable_error(
            "Mood analysis not available - missing dependencies"
        )
...
def handle_mood_tags(validated_data):
    ...
    try:
        ...
        logger.error(
            f"Unexpected error in handle_mood_tags: {type(e).__name__}: {e}",
            exc_info=True,
        )
        return handle_exception(e, "handle_mood_tags")
...
def handle_mood_search(validated_data):
    ...
    try:
        ...
        try:
            from mood_analysis.mood_query_parser import (
                parse_mood_query,
            ...
            return success_response(
                data={"recommendations": recommendations, "query": mood_query}
            )
...
def handle_generate_note(validated_data):
    ...
    try:
        ...
        # Check cache
        cached_note = BookNote.query.filter_by(
        ...
        try:
            if recommendation and isinstance(recommendation, dict):
                ...
                new_note = BookNote(
                    book_title=title, book_author=author, content=blurb_content
                )
        ...
        logger.error(
            f"Unexpected error in handle_generate_note: {type(e).__name__}: {e}",
            exc_info=True,
        )
        return handle_exception(e, "handle_generate_note")
...
def handle_socket_chat(data):
    ...
    try:
        ...
        if not is_valid:
            emit(
                "chat_error", {"error": "Validation failed", "details": validated_data}
            )
            return
        ...
        logger.error(
            f"Error in WebSocket handle_chat: {type(e).__name__}: {e}", exc_info=True
        )
        emit(
            "chat_error",
            {
    ...
    try:
        ...
        if not book:
            ...
            try:
                ...
                author_str = (
                    ", ".join(validated_data.authors)
                    if isinstance(validated_data.authors, list)
                    else validated_data.authors
                )
                clean_id = "".join(
                    [
                        c if c.isalnum() else "_"
                        for c in validated_data.title.lower().strip()
                    ]
                )
        ...
        existing_item = (
            ShelfItem.query.filter_by(user_id=validated_data.user_id, book_id=book.id)
            .with_for_update()
            .first()
        )
        if existing_item:
...
def get_library(user_id):
    ...
    try:
        items = (
            ShelfItem.query.options(joinedload(ShelfItem.book))
...
def _update_reading_stats(user_id, book):
    ...
    if not stats:
        stats = ReadingStats(
            user_id=user_id, year=year, month=month, books_completed=0, pages_read=0
        )
...
def _calculate_reading_streak(user_id):
    ...
    """
    finished_items = (
        ShelfItem.query.filter_by(user_id=user_id, shelf_type="finished")
        .filter(ShelfItem.finished_at.isnot(None))
        .order_by(ShelfItem.finished_at.desc())
        .all()
    )
...
def update_library_item(item_id, validated_data):
    ...
    try:
        ...
        if (
            validated_data.version is not None
            and item.version != validated_data.version
        ):
        ...
        db.session.commit()
        return success_response(
            data={"message": "Item updated", "item": item.to_dict()}
        )
    except SQLAlchemyError as e:
...
def sync_library(validated_data):
    ...
    try:
        ...
        for index, item_data in enumerate(validated_data.items):
            ...
            if raw_google_id is None or not validate_google_books_id(
                str(raw_google_id).strip()
            ):
            ...
            try:
                with db.session.begin_nested():
                    ...
                    if not book:
                        ...
                        try:
                            ...
                            clean_id = "".join(
                                [
                                    c if c.isalnum() else "_"
                                    for c in sync_title.lower().strip()
                                ]
                            )
                            create_spine(sync_title, authors, clean_id)
                        except Exception as spine_err:
                            logger.error(
                                f"Spine generation failed during bulk sync: {spine_err}"
                            )
                        # -----------------------------------------------

                    existing_item = (
                        ShelfItem.query.filter_by(user_id=user_id, book_id=book.id)
                        .with_for_update()
                        .first()
                    )
                    ...
                    if not existing_item:
                        ...
                        existing_item.progress = item_data.get(
                            "progress", existing_item.progress
                        )
                        existing_item.version += 1
                        synced_count += 1
                ...
                logger.error(
                    f"Database error syncing item {item_data.get('id', 'unknown')}: {e}"
                )
                errors += 1
            except Exception as e:
                logger.error(
                    f"Unexpected error syncing item {item_data.get('id', 'unknown')}: {e}"
                )
                errors += 1
        ...
        try:
            ...
            if not user:
                return internal_error(
                    "Failed to create user record after registration."
                )
...
def login(validated_data):
    ...
    try:
        ...
        logger.error(
            f"Unexpected error in login: {type(e).__name__}: {e}", exc_info=True
        )
...
def google_login():
    ...
    google_redirect_uri = app.config.get("GOOGLE_OAUTH_REDIRECT_URI") or url_for(
        "google_oauth_callback", _external=True
    )
    google_scope = app.config.get(
        "GOOGLE_OAUTH_SCOPE",
        "openid email profile https://www.googleapis.com/auth/books",
    )
...
def google_oauth_callback():
    ...
    frontend_redirect_url = app.config.get(
        "GOOGLE_OAUTH_FRONTEND_REDIRECT_URL",
        "http://127.0.0.1:5500/frontend/pages/library.html",
    )
    ...
    try:
        ...
        if not user:
            ...
            if user:
                ...
                base_username = "".join(
                    ch
                    for ch in username.strip().replace(" ", "_")
...
UPLOAD_FOLDER = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "uploads", "avatars"
)
...
def upload_avatar(user_id):
    ...
    try:
        ...
        if file_mime not in ALLOWED_MIME_TYPES:
            return (
                jsonify({"error": "Invalid file content (magic number mismatch)"}),
                400,
            )
    except Exception as e:
...
def set_reading_goal(validated_data):
    ...
    try:
        ...
        return (
            jsonify(
                {"message": "Reading goal set successfully", "goal": goal.to_dict()}
            ),
            200,
        )
    except Exception as e:
            ...
            jsonify(
                {
                    ...
                    "books_this_month": (
                        current_month_stats.books_completed
                        if current_month_stats
                        else 0
                    ),
                    "pages_this_month": (
                        current_month_stats.pages_read if current_month_stats else 0
                    ),
    ...
    try:
        ...
        stats_query = (
            db.session.query(
                ...
                func.coalesce(func.sum(ReadingStats.books_completed), 0).label(
                    "total_books"
                ),
                func.coalesce(func.sum(ReadingStats.pages_read), 0).label(
                    "total_pages"
                ),
            )
            ...
            .outerjoin(
                ReadingStats,
                (ReadingGoal.user_id == ReadingStats.user_id)
                & (ReadingStats.year == year),
            )
            .filter(ReadingGoal.year == year)
            .group_by(ReadingGoal.user_id, User.username, ReadingGoal.target_books)
            .all()
        )
            ...
            leaderboard.append(
                {
                    ...
                    "progress_percentage": (
                        round((total_books_val / target_books * 100), 1)
                        if target_books > 0
                        else 0
                    ),
                }
...
def update_collection(collection_id, validated_data):
    ...
    try:
        ...
        if validated_data.name:
            ...
            if existing:
                return (
                    jsonify({"error": "Collection with this name already exists"}),
                    409,
                )
            collection.name = validated_data.name
...
def add_book_to_collection(collection_id):
    ...
    try:
        ...
        return (
            jsonify({"message": "Book added to collection", "item": item.to_dict()}),
            201,
        )
    except Exception as e:
        db.session.rollback()
...
def get_collection_books(collection_id):
    ...
    try:
        ...
        items = (
            CollectionItem.query.filter_by(collection_id=collection_id)
...
@app.route(
    "/api/v1/collections/<int:collection_id>/books/<int:book_id>", methods=["DELETE"]
)
@jwt_required()
...
def create_or_update_review():
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"message": message, "review": review.to_dict()}), (
...
def create_price_alert(book_id):
    ...
    try:
        ...
        if shelf_item.book_id != book.id:
            return (
                jsonify({"error": "Shelf item does not match the specified book"}),
                400,
            )
        ...
        if result.get("success"):
            ...
            return (
                jsonify({"error": result.get("error", "Failed to create price alert")}),
                400,
            )
...
def get_price_history(book_id):
    ...
    try:
        ...
        history = price_tracker.get_price_history(
            book_id=book.id, retailer=retailer, limit=limit
        )
...
def delete_price_alert(alert_id):
    ...
    try:
        ...
        result = price_tracker.delete_price_alert(
            alert_id=alert_id, user_id=current_user_id
        )
        ...
        if result.get("success"):
            ...
            return (
                jsonify({"error": result.get("error", "Failed to delete price alert")}),
                400,
            )
...
if __name__ == "__main__":
    ...
    with app.app_context():
        ...
        logger.info(
            "  GET  /api/v1/library/collections/<id>/items - Get collection items"
        )
        logger.info(
            "  POST /api/v1/library/collections/<id>/items - Add item to collection"
        )
        ...
        logger.info(
            "  POST /api/v1/chat - Chat with bookseller (DEPRECATED - Use WebSockets)"
        )
        logger.info("  GET  /api/v1/health - Health check")

    socketio.run(
        app, debug=server_config.debug, port=server_config.port, host=server_config.host
    )
