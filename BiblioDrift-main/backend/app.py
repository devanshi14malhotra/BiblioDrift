...
def get_collection_books(collection_id):
    ...
    try:
        ...
        # FIX: Use joinedload to fetch books in ONE query instead of N queries
        items = CollectionItem.query.options(
            joinedload(CollectionItem.book)
        ).filter_by(collection_id=collection_id).order_by(CollectionItem.added_at.desc()).all()
...
def get_public_collections():
    ...
    try:
        ...
        # FIX: Use joinedload to fetch users in ONE query instead of N queries
        ...
        for c in collections:
            ...
            # User is already loaded in memory from joinedload - no additional query!
...
def get_book_reviews(book_id):
    ...
    try:
        ...
        # FIX: Use joinedload to fetch users in ONE query instead of N queries
        # This ensures that when review.to_dict() accesses review.user, it's already loaded
        reviews = Review.query.options(
            joinedload(Review.user),
            joinedload(Review.book)
        ).filter_by(book_id=book.id).order_by(Review.created_at.desc()).all()
...
def get_user_reviews(user_id):
    ...
    try:
        # FIX: Use joinedload to fetch book info in ONE query instead of N queries
        # This ensures that when review.to_dict() accesses review.book, it's already loaded
        reviews = Review.query.options(
            joinedload(Review.book),
            joinedload(Review.user)
        ).filter_by(user_id=user_id).order_by(Review.created_at.desc()).all()
